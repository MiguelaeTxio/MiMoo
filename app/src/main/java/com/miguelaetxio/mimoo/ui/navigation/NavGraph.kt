...
sealed class Screen(val route: String) {
    ...
    object ArtistForm   : Screen("artists/form?artistId={artistId}") {
        fun createRoute(artistId: Long? = null) =
            if (artistId != null) "artists/form?artistId=$artistId" else "artists/form"
    }
    object AlbumList    : Screen("artists/{artistId}/albums") {
        fun createRoute(artistId: Long) = "artists/$artistId/albums"
    }
    object AlbumForm    : Screen("artists/{artistId}/albums/form?albumId={albumId}") {
        fun createRoute(artistId: Long, albumId: Long? = null) =
            if (albumId != null) "artists/$artistId/albums/form?albumId=$albumId"
...
fun MiMooNavGraph(navController: NavHostController) {
    NavHost(navController = navController, startDestination = Screen.ArtistList.route) {

        composable(Screen.ArtistList.route) {
            ArtistListScreen(
                onArtistClick  = { id -> navController.navigate(Screen.ArtistDetail.createRoute(id)) },
                onAddArtist    = { navController.navigate(Screen.ArtistForm.createRoute()) },
            )
        }
        ...
        composable(
            ...
            ArtistDetailScreen(
                ...
                onEdit         = { id -> navController.navigate(Screen.ArtistForm.createRoute(id)) },
                onViewAlbums   = { id -> navController.navigate(Screen.AlbumList.createRoute(id)) },
            )
        }
        ...
        composable(
            ...
            AlbumListScreen(
                onBack     = { navController.popBackStack() },
                onAddAlbum = { artistId -> navController.navigate(Screen.AlbumForm.createRoute(artistId)) },
                onAlbumEdit = { artistId, albumId -> navController.navigate(Screen.AlbumForm.createRoute(artistId, albumId)) },
            )
        }
